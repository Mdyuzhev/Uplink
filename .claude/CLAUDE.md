...
- **WASM:** prebuild скрипт `web/scripts/copy-wasm.cjs` копирует .wasm в public/ (запускается автоматически перед build). Dockerfile тоже копирует (дубль, не ошибка). VS Code WebView: fetch-интерсептор в UplinkPanel.ts перенаправляет на vscode-resource URI
...
| 2026-03-07 | task_047 | Inline-кнопки для SDK-ботов (Telegram-style): uplink.buttons в сообщениях, callback-событие uplink.bot.callback, edit_message action в botGateway, Matrix m.replace для редактирования, E2E дисклеймер в BotCreateModal |
| 2026-03-07 | task_048 | Голосовые каналы (Discord-style): VoiceRoomInfo, uplink.room.type=voice, uplink.voice.member presence, useVoiceChannel хук (join/leave/mute через LiveKit), VoiceChannelItem + VoiceBar компоненты, секция в сайдбаре, beforeunload cleanup, скрипт create-voice-channel.sh |
| 2026-03-07 | task_049 | Inline-плеер для видео: m.video → type:'video', HTML5 `<video>` с controls/poster/preload, мета-строка (имя+размер+скачать), CSS в chat.css |
| 2026-03-07 | task_050 | Контекстное меню (ПКМ): MessageContextMenu (Portal), quick emoji strip, ответить/тред/закрепить/копировать/скачать/открыть, удаление (redactEvent), onDelete через стек MessageList→ChatLayout→useChatState |
| 2026-03-07 | task_051 | Embed-превью видео: YouTube (watch/shorts/youtu.be), RuTube, VK Video — inline iframe 16:9 в текстовых сообщениях, detectVideoEmbed в MessageFormatter, CSS в chat.css |
| 2026-03-07 | task_052 | Проверка обновлений: useUpdateCheck хук (GitHub Releases API, semver compare), секция «О приложении» в ProfileModal, __APP_VERSION__ через Vite define, tauri-plugin-shell для внешних ссылок, macOS Intel target в CI |
| 2026-03-08 | task_053 | Системные звуки: SoundService (Web Audio API синтез), звуки message/mention/call/send, тогглер в ProfileModal, замена callSounds.ts |
| 2026-03-08 | task_054 | WASM fix для VS Code: prebuild скрипт (copy-wasm.cjs), fetch-интерсептор в UplinkPanel.ts (vscode-resource URI), CSP connect-src fix |
